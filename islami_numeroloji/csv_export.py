...
from .hesaplama_cakra import cakra_metin_hesaplamalari, cocuk_cakra_metin_hesaplamalari_ayrintili, cakra_metin_hesaplamalari_ayrintili
...
def generate_csv_export(data):
    ...
    """
    try:
        logger.debug("CSV export başladı")
        ...
        logger.debug(f"Form verileri: {data}")
        ...
        logger.debug(f"İşlenecek veriler: dogum_gunu={dogum_gunu}, isim={isim_soyisim}, cinsiyet={cinsiyet}")
        
        # Hesaplamaları yap
        logger.debug("Yaş hesaplanıyor...")
        yas = yas_hesapla(dogum_gunu)
        logger.debug(f"Yaş: {yas}")
        
        logger.debug("Yaşam yolu hesaplanıyor...")
        yasam_yolu = yasam_yolu_hesapla(dogum_gunu)
        logger.debug(f"Yaşam yolu: {yasam_yolu}")
        
        logger.debug("Pin kodu hesaplanıyor...")
        pin_kodu = pin_kodu_hesaplama(dogum_gunu)
        logger.debug(f"Pin kodu: {pin_kodu}")
        
        logger.debug("Chakra hesaplanıyor...")
        arti_sistemi, chakra_result = chakra_hesapla(pin_kodu, isim_soyisim, eklenen_isim)
        logger.debug(f"Arti sistemi: {arti_sistemi}")
        
        logger.debug("Pin kodu yorumları alınıyor...")
        pin_kodu_yorumlari = pin_kodu_yorumlari_algoritmasi(pin_kodu, arti_sistemi, cinsiyet, yasam_yolu)
        logger.debug(f"Pin kodu yorumları sayısı: {len(pin_kodu_yorumlari) if isinstance(pin_kodu_yorumlari, list) else 'N/A'}")
        
        # Yaşa göre çakra hesaplama seç
        logger.debug("Çakra hesaplama seçiliyor...")
        if yas is not None and yas <= 5:
            logger.debug("Çocuk çakra hesaplaması seçildi")
            try:
                logger.debug("cocuk_cakra_metin_hesaplamalari_ayrintili çağrılıyor...")
                result = cocuk_cakra_metin_hesaplamalari_ayrintili(arti_sistemi)
                logger.debug(f"Çocuk çakra sonucu: {type(result)}, uzunluk: {len(result) if isinstance(result, tuple) else 'N/A'}")
                logger.debug(f"Çocuk çakra sonucu detay: {result}")
                
                if len(result) == 3:
                    cakra_metinleri, evlilik_metinleri, alma_verme_dengesi = result
                elif len(result) == 2:
                    cakra_metinleri, alma_verme_dengesi = result
                    evlilik_metinleri = []
                    logger.warning("Çocuk çakra fonksiyonu 2 değer döndürdü, evlilik_metinleri boş liste olarak ayarlandı")
                else:
                    raise ValueError(f"Çocuk çakra fonksiyonu beklenmeyen sayıda değer döndürdü: {len(result)}")
                    
                logger.debug(f"Çocuk çakra metinleri: {len(cakra_metinleri) if cakra_metinleri else 0} adet")
                logger.debug(f"Çocuk evlilik metinleri: {len(evlilik_metinleri) if evlilik_metinleri else 0} adet")
                logger.debug(f"Çocuk alma verme dengesi: {alma_verme_dengesi}")
                
            except Exception as e:
                logger.error(f"Çocuk çakra hesaplama hatası: {str(e)}")
                logger.error(f"Hata detayı: {type(e).__name__}")
                import traceback
                logger.error(f"Traceback: {traceback.format_exc()}")
                raise
        else:
            logger.debug("Yetişkin çakra hesaplaması seçildi")
            cakra_metinleri, evlilik_metinleri, alma_verme_dengesi = cakra_metin_hesaplamalari_ayrintili(arti_sistemi, cinsiyet)
            logger.debug(f"Yetişkin çakra metinleri: {len(cakra_metinleri) if cakra_metinleri else 0} adet")
        
        logger.debug("Merkez sayı hesaplanıyor...")
        # Merkez sayının tuple değerini al
        merkez_sayi_detay, merkez_sayi_integer = merkez_sayi_bulma(isim_soyisim)
        logger.debug(f"Merkez sayı: {merkez_sayi_detay}")
        
        logger.debug("Pin kodu özellikleri hesaplanıyor...")
        pin_kodu_ozellikleri = ozellik_hesaplama(pin_kodu)
        
        logger.debug("Yaşam yolu açıklaması alınıyor...")
        ...
        logger.debug("Çakra metinleri işleniyor...")
        ...
        logger.debug("Çakra metinleri sayfalara bölünüyor...")
        # Şimdi çakra metinlerini ekle
        if cakra_metinleri:
            for i in range(9):
                if i < len(cakra_metinleri):
                    metin = cakra_metinleri[i]
                    logger.debug(f"Çakra {i+1} metni işleniyor, uzunluk: {len(metin) if metin else 0}")
            ...
            logger.warning("Çakra metinleri bulunamadı!")
        
        logger.debug("Pin kodu yorumları işleniyor...")
        ...
        logger.debug("Yaşam yolu açıklaması işleniyor...")
        ...
        logger.debug("Merkez sayı açıklaması işleniyor...")
        ...
        logger.debug("Alma verme dengesi işleniyor...")
        ...
        logger.debug("Evlilik metinleri işleniyor...")
        ...
        logger.debug("Evlilik ek metinleri işleniyor...")
        # Evlilik ek metinleri - kadın ve ek isim varsa çağır, erkek ve ek isim yoksa boş string
        evlilik_ek_metinleri = []
        
        # Cinsiyet kontrolü - hem 'K' hem 'kadın' olabilir
        is_female = cinsiyet.lower() in ['k', 'kadın', 'kadin']
        has_ek_isim = eklenen_isim and eklenen_isim.strip()
        
        logger.debug(f"Cinsiyet: {cinsiyet}, Ek isim: '{eklenen_isim}', Kadın mı: {is_female}, Ek isim var mı: {has_ek_isim}")
        
        if is_female and has_ek_isim:  # Kadın ve ek isim varsa
            try:
                evlilik_ek_metinleri = evlilik_ek_metin(eklenen_isim)
                logger.debug(f"Evlilik ek metinleri alındı: {len(evlilik_ek_metinleri)} adet")
                logger.debug(f"Evlilik ek metinleri içeriği: {evlilik_ek_metinleri}")
            except Exception as e:
                logger.error(f"Evlilik ek metin hatası: {str(e)}")
                evlilik_ek_metinleri = [".", "."]  # Hata durumunda boş sayfalar
        else:
            # Erkek veya ek isim yoksa boş string
            evlilik_ek_metinleri = [".", "."]
            logger.debug("Erkek veya ek isim yok, boş metinler kullanılıyor")
        
        # Evlilik ek metinleri için 2 sayfa oluştur
        evlilik_ek_pages = {}
        if len(evlilik_ek_metinleri) >= 2:
            evlilik_ek_pages['_sayfa_1'] = evlilik_ek_metinleri[0]
            evlilik_ek_pages['_sayfa_2'] = evlilik_ek_metinleri[1]
        else:
            # Eğer 2'den az metin varsa, eksik olanı boş string ile doldur
            evlilik_ek_pages['_sayfa_1'] = evlilik_ek_metinleri[0] if len(evlilik_ek_metinleri) > 0 else ""
            evlilik_ek_pages['_sayfa_2'] = evlilik_ek_metinleri[1] if len(evlilik_ek_metinleri) > 1 else ""
        
        logger.debug("CSV oluşturuluyor...")
        ...
        # Evlilik ek metinleri başlıkları (2 sayfa)
        headers.extend([
            'evlilik_ek_metin_sayfa_1',
            'evlilik_ek_metin_sayfa_2'
        ])
        ...
        logger.debug(f"CSV başlıkları oluşturuldu: {len(headers)} adet")
        ...
        logger.debug("Veri satırı oluşturuluyor...")
        # Veri satırı
        row_data = [
        ...
        logger.debug("Çakra metinleri ekleniyor...")
        ...
        logger.debug("Pin kodu yorumları ekleniyor...")
        ...
        logger.debug("Yaşam yolu açıklaması ekleniyor...")
        ...
        logger.debug("Merkez sayı ekleniyor...")
        # Merkez sayı
        row_data.append(merkez_sayi_detay or '')
        
        logger.debug("Merkez sayı açıklaması ekleniyor...")
        ...
        logger.debug("Alma verme dengesi ekleniyor...")
        ...
        logger.debug("Evlilik metinleri ekleniyor...")
        ...
        logger.debug("Evlilik ek metinleri ekleniyor...")
        # Evlilik ek metinleri sayfalarını ekle (2 sayfa)
        evlilik_ek_page_1 = evlilik_ek_pages.get('_sayfa_1', '')
        evlilik_ek_page_2 = evlilik_ek_pages.get('_sayfa_2', '')
        
        logger.debug(f"Evlilik ek metin sayfa 1: '{evlilik_ek_page_1[:100]}...' (uzunluk: {len(evlilik_ek_page_1)})")
        logger.debug(f"Evlilik ek metin sayfa 2: '{evlilik_ek_page_2[:100]}...' (uzunluk: {len(evlilik_ek_page_2)})")
        
        row_data.extend([evlilik_ek_page_1, evlilik_ek_page_2])
        logger.debug(f"Evlilik ek metinleri row_data'ya eklendi. Toplam veri sayısı: {len(row_data)}")
        ...
        logger.debug(f"Veri satırı oluşturuldu: {len(row_data)} adet veri")
        
        # Veri satırını yaz
        writer.writerow(row_data)
        
        logger.debug("CSV içeriği alınıyor...")
        # CSV içeriğini al
        ...
        logger.debug(f"CSV dosyası oluşturuldu: {filename}")
        ...
        logger.debug("CSV export başarıyla tamamlandı")
        return response
